...
**Acceptance criteria:**
- [x] 输出 AI 原始候选正确率、管理员修改率、编译成功率和留出复用正确率。
- [x] 任务 64/65/66 满足全部面单覆盖守恒。
- [x] 正常供应商 Excel 的业务字段、行数、图片与冻结基准一致；3 条新增子商品行已由原始 `printXML` 逐条证明为 5173 漏行。
- [x] AI 服务断开时已学格式继续工作，陌生格式进入明确异常。
- [x] 6173 最终镜像、Git、数据库和工作簿均有阶段回退点。

**Verification:**
- [x] 后端全量测试、前端类型检查、容器健康和浏览器验收通过。
- [x] 独立代码审查无阻塞项。
- [x] 5173 前后容器 ID、restart count 和健康状态一致。
...
## Final 6173 Acceptance (2026-07-31)

- 5173 四个现场容器的 ID、镜像、运行状态和 `RestartCount=0` 在验收前后保持不变。
- 第三套真零规则数据卷保留 3 个采集任务、102 条原始记录和全部商品、SKU、图片、档口及匹配资产。
- 25 次本地 4B 会话全部由真实页面完成；25 条规则均编译成功并精确回放确认样本。
- 4B 原始候选整单正确率为 0/25、五字段一致率为 38.26%；发布结构据此固定为“4B 只给候选，管理员修正一次，确定性编译器生成规则”，不允许模型直接写规则。
- 已学规则自动覆盖 281/283 张面单且不再调用模型；剩余 2 张缺少商品字段，按失败关闭进入异常。
- 任务 64/65/66 共 283 张面单全部覆盖，规则生成 293 条商品行；266 条进入正常报货，27 条以可操作原因进入异常，无去重、无漏单。
- 实际下载的任务 66 Excel 含 84 条正常行、84 张图片和 13 条异常；所有正常页均为七列，异常页仅有“图片匹配文本”。
- AI 服务停止期间，三个任务的覆盖、正常行和异常行统计完全不变；AI 恢复后继续可用。
- 已验证从发布镜像回退到上一阶段镜像、再恢复发布镜像，数据卷未更换，最终统计不变。
- 事后冻结基准比较中，当前正常行核心四字段 263/266 与 5173 一致；其余结果未静默猜测，均保留为正常新拆行或明确异常，不把基准答案回灌规则学习。
