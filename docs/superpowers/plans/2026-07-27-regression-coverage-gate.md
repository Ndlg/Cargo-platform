...
- [x] 复用现有任务、原始记录、面单拆分和识别结果函数。
- [x] 默认扫描已结束任务；支持重复传入 `--task-id`。
- [x] 只输出计数和任务 ID；硬失败返回退出码 1。
- [x] 运行 `--help` 和 6173 副本冒烟检查。

### Task 3: 全量验证与收口

- [x] 运行完整后端测试。
- [x] 在 6173 数据副本扫描全部历史已结束任务。
- [x] 检查输出不含业务原文。
- [x] 更新 `tasks/todo.md` 和扫描记录。
- [x] 保留隔离分支，未获确认前不合并、不部署。
